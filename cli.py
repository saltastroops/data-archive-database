...
from ssda.database_update import (
    UpdateAction,
    fits_data_from_date_range_gen,
...
class DateWithKeywordsParamType(click.ParamType):
    ...
    def convert(self, value: str, param, ctx):
        ...
        try:
            ...
            self.fail(
                f"{value} is neither a date of the form yyyy-mm-dd, nor the string yesterday, nor the "
                f"string last-year."
            )
...
def validate_options(
    ...
    * A future date is specified.
    ...
    if file and len(set(instruments)) != 1:
        raise Exception(
            "The --instrument option must be used exactly once if you use the --file option."
        )
...
def update_database(
    ...
    try:
        ...
        if start and end:
            logging.info(
                "Files from {start} to {end} are considered.".format(
                    start=start.strftime("%d %b %Y"), end=end.strftime("%d %b %Y")
                )
            )
            fits_data_gen = fits_data_from_date_range_gen(
                start_date=start, end_date=end, instruments=set(instruments)
            )
        elif file:
            fits_data_gen = fits_data_from_file_gen(
                fits_file=file, instrument=list(instruments)[0]
            )
        else:
...
@click.group()
@click.option("--verbose", is_flag=True, help="Log in verbose mode.")
def cli(verbose):
    """
...
@click.option(
    "--force", is_flag=True, help="Do not ask for confirmation before deleting."
)
@click.option(
    "--instrument",
    "instruments",
...
def delete(
    end: datetime, file: str, force: bool, instruments: Tuple[str], start: datetime
):
    """
    ...
    if not force and not click.confirm(
        "Do you really want to delete database entries?"
    ):
