...
CREATE TABLE `TargetType` (
 `targetTypeId` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary key.',
 `numericValue` VARCHAR(255) DEFAULT NULL COMMENT 'The numeric code for the target type, such as “14.06.16.01”.',
 `explanation` VARCHAR(100) DEFAULT NULL COMMENT 'An explanation of the target type, such as “Pulsating White Dwarf”.',
...
CREATE TABLE `Target` (
 `targetId` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary key.',
 `name` VARCHAR(255) DEFAULT NULL COMMENT 'target name',
 `rightAscension` float DEFAULT NULL COMMENT 'target right ascension',
 `declination` float DEFAULT NULL COMMENT 'target declination',
 `position` point SRID 123456 DEFAULT NULL COMMENT 'RA is offset by -180 deg so that it falls into [-180, 180]',
 `targetTypeId` int(11) DEFAULT NULL COMMENT 'A foreign key linking to target type, see targetType table',
...
CREATE TABLE `Institution` (
 `institutionId` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary key.',
 `institutionName` VARCHAR(255) DEFAULT NULL COMMENT 'Institute name like SAAO, SALT',
...
CREATE TABLE `Telescope` (
 `telescopeId` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary key.',
 `telescopeName` VARCHAR(255) NOT NULL  COMMENT 'Name given to a telescope',
 `ownerId` int(11) DEFAULT NULL COMMENT 'The id of the institution owning the telescope. This links to the Institution table.',
...
CREATE TABLE `Proposal` (
 `proposalId` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary key.',
 `proposalCode` VARCHAR(255) NOT NULL COMMENT 'The code used for referring to a proposal, such as 2018-2-SCI-042.',
 `principalInvestigatorGivenName` VARCHAR(255) NOT NULL COMMENT 'The principal investigator’s given name.',
 `principalInvestigatorFamilyName` VARCHAR(255) NOT NULL COMMENT 'The principal investigator’s family name.',
 `title` VARCHAR(255) NOT NULL COMMENT 'The proposal title',
 `institutionId` INT(11) NOT NULL COMMENT 'A foreign key linking to Institute table',
 `lastUpdated` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'timestamp of when it was last updated',
...
CREATE TABLE `ObservationStatus` (
 `observationStatusId` int(11) NOT NULL COMMENT 'Primary key.',
 `status` VARCHAR(255) DEFAULT NULL COMMENT 'The observation status. The possible values are Accepted, Rejected and Unknown',
...
CREATE TABLE `Observation` (
 `observationId` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary key.',
 `proposalId` int(11) COMMENT 'A foreign key linking to proposal, see table Proposal',
 `telescopeId` int(11) DEFAULT NULL  COMMENT 'A foreign key linking to telescope, see table Telescope',
 `telescopeObservationId` VARCHAR(255) DEFAULT NULL COMMENT 'Same as a block id for SALT, it is not a key of ssda',
 `night` VARCHAR(255) DEFAULT NULL COMMENT 'date an observation was taken',
 `observationStatusId` int(11) DEFAULT NULL COMMENT 'A foreign key linking to observation status, see table ObservationStatus',
 ...
 */
 ...
 CREATE TABLE `DataCategory` (
  `dataCategoryId` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary key.',
  `dataCategory` VARCHAR(255) DEFAULT NULL COMMENT 'The data category. Possible values are Science, Flat, Dark, Arc, Bias, Engineering and Documentation',
...
CREATE TABLE `DataFile` (
 `dataFileId` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Unique key',
 `dataCategoryId` int(11) DEFAULT NULL COMMENT 'A foreign key linking to data category see table `DataCategory`',
 `startTime` DATETIME DEFAULT NULL COMMENT 'Fits file "observation time" recorded on file',
 `name` VARCHAR(255) DEFAULT NULL COMMENT 'Filename i.e R20190203000023.fits',
 `path` VARCHAR(255) DEFAULT NULL COMMENT 'A file paths i.e /home/path/to/file/raw/R20190203000023.fits',
 `targetId` int(11) DEFAULT NULL COMMENT 'A foreign key linking target see table Target',
 `size` FLOAT DEFAULT NULL COMMENT 'File size in kilobytes',
 `observationId` int(11) DEFAULT NULL COMMENT 'A foreign key linking to observation, see table Observation',
...
CREATE TABLE `DataPreview` (
 `dataPreviewId` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary key.',
 `name` VARCHAR(255) DEFAULT NULL,
 `path` VARCHAR(255) DEFAULT NULL COMMENT 'Path to preview path',
 `dataFileId` int(11) DEFAULT NULL COMMENT 'A foreign key linking to data file, see table DataFile',
 `order` VARCHAR(255) DEFAULT NULL COMMENT 'Defines an order within multiple preview files for the same data file',
...
CREATE TABLE `HRS` (
 `hrsId` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary key for this table(HRS).',
 `dataFileId` int(11) NOT NULL COMMENT 'A foreign key linking to data file, see table DataFile',
...
CREATE TABLE `RSS` (
 `rssId` INT(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary key for this table(RSS).',
 `dataFileId` INT(11) NOT NULL COMMENT 'A foreign key linking to DataFile table.',
 `amplifierSection` VARCHAR(255) DEFAULT NULL,
 `amplifierTemperature` FLOAT DEFAULT NULL,
...
CREATE TABLE `Salticam` (
 `salticamId` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary key for this table(Salticam).',
 `dataFileId` int(11) NOT NULL COMMENT 'A foreign key linking to Datafile table.',
 `amplifierSection` VARCHAR(255) DEFAULT NULL,
 `amplifierTemperature` FLOAT DEFAULT NULL,
 `amplifierReadoutX` INT DEFAULT NULL,
...
CREATE TABLE `ProposalInvestigator` (
 `proposalId` int(11) NOT NULL COMMENT 'A foreign key linking to proposal, see Proposal table',
