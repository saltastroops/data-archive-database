...
class ObservationPropertiesStub(ObservationProperties):
    def artifact(self, plane_id: int) -> types.Artifact:
        return types.Artifact(content_checksum='chu346jh',
    ...
    def energy(self, plane_id: int) -> types.Energy:
        return types.Energy(dimension=512,
    ...
    def instrument_keyword_values(self, observation_id: int) -> List[types.InstrumentKeywordValue]:
        return [types.InstrumentKeywordValue(instrument=types.Instrument.RSS, instrument_keyword=types.InstrumentKeyword.GRATING, observation_id=observation_id, value='pg1300'),
                                 types.InstrumentKeywordValue(instrument=types.Instrument.RSS, instrument_keyword=types.InstrumentKeyword.BANDPASS, observation_id=observation_id, value='some_filter')]


    def observation(self, proposal_id: int) -> types.Observation:
        return types.Observation(data_release=date(2020, 1, 1),
    ...
    def observation_time(self, plane_id: int) -> types.ObservationTime:
        return types.ObservationTime(end_time=datetime(2019, 9, 7, 23, 10, 10),
    ...
    def polarizations(self, plane_id: int) -> List[types.Polarization]:
        return [types.Polarization(plane_id=plane_id, stokes_parameter=types.StokesParameter.Q),
                     types.Polarization(plane_id=plane_id, stokes_parameter=types.StokesParameter.U)]


    def position(self, plane_id: int) -> types.Position:
        return types.Position(dec=-23.7 * u.degree, equinox=2000, plane_id=plane_id, ra=245.9 * u.deg)


    def proposal(self) -> types.Proposal:
        return types.Proposal(institution=types.Institution.SALT,
                              pi='John Doe',
    ...
    def proposal_investigators(self, proposal_id: int) -> List[types.ProposalInvestigator]:
        return [types.ProposalInvestigator(proposal_id=proposal_id, investigator_id='a54'),
    ...
    def target(self, observation_id: int) -> types.Target:
        return types.Target(name='Some Interesting Target',
...
def test_all_content_is_inserted(mocker):
    ...
    mock_database_service.return_value.insert_instrument_keyword_value.side_effect = [1409, 1410]
    ...
    assert_equal_properties(mock_database_service.return_value.insert_proposal.call_args[0][0], observation_properties.proposal())

    # proposal investigators inserted
    assert mock_database_service.return_value.insert_proposal_investigator.call_count == 3
    for i  in range(3):
        assert_equal_properties(mock_database_service.return_value.insert_proposal_investigator.call_args_list[i][0][0], observation_properties.proposal_investigators(PROPOSAL_ID)[i])

    # observation inserted
    mock_database_service.return_value.insert_observation.assert_called_once()
    assert_equal_properties(mock_database_service.return_value.insert_observation.call_args[0][0], observation_properties.observation(PROPOSAL_ID))

    # target inserted
    mock_database_service.return_value.insert_target.assert_called_once()
    assert_equal_properties(mock_database_service.return_value.insert_target.call_args[0][0], observation_properties.target(OBSERVATION_ID))

    # instrument keyword values inserted
    assert mock_database_service.return_value.insert_instrument_keyword_value.call_count == 2
    for i in range(2):
        assert_equal_properties(mock_database_service.return_value.insert_instrument_keyword_value.call_args_list[i][0][0], observation_properties.instrument_keyword_values(OBSERVATION_ID)[i])

    # plane inserted
    mock_database_service.return_value.insert_plane.assert_called_once()
    assert_equal_properties(mock_database_service.return_value.insert_plane.call_args[0][0], observation_properties.plane(OBSERVATION_ID))

    # energy inserted
    mock_database_service.return_value.insert_energy.assert_called_once()
    assert_equal_properties(mock_database_service.return_value.insert_energy.call_args[0][0], observation_properties.energy(PLANE_ID))

    # polarizations inserted
    assert mock_database_service.return_value.insert_polarization.call_count == 2
    for i  in range(2):
        assert_equal_properties(mock_database_service.return_value.insert_polarization.call_args_list[i][0][0], observation_properties.polarizations(PLANE_ID)[i])

    # observation time inserted
    mock_database_service.return_value.insert_observation_time.assert_called_once()
    assert_equal_properties(mock_database_service.return_value.insert_observation_time.call_args[0][0],
                            observation_properties.observation_time(PLANE_ID))

    # position inserted
    mock_database_service.return_value.insert_position.assert_called_once()
    assert_equal_properties(mock_database_service.return_value.insert_position.call_args[0][0], observation_properties.position(PLANE_ID))

    # artifact inserted
    mock_database_service.return_value.insert_artifact.assert_called_once()
    assert_equal_properties(mock_database_service.return_value.insert_artifact.call_args[0][0], observation_properties.artifact(PLANE_ID))
...
def test_proposals_are_not_reinserted(mocker):
    ...
    assert mock_database_service.return_value.insert_proposal_investigator.asserrt_not_called()
...
def test_transactions_are_rolled_back_in_case_of_an_error(mocker):
    ...
    mock_database_service.return_value.find_proposal_id.return_value = None
    mock_database_service.return_value.insert_artifact.return_value = 713
    mock_database_service.return_value.insert_energy.side_effect = ValueError('This is a fake error.')
    mock_database_service.return_value.insert_instrument_keyword_value.side_effect = [1409, 1410]
    mock_database_service.return_value.insert_observation.return_value = OBSERVATION_ID
    mock_database_service.return_value.insert_observation_time.return_value = 23011
    mock_database_service.return_value.insert_plane.return_value = PLANE_ID
