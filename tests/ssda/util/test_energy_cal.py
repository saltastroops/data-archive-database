...
from ssda.util.energy_cal import fwhm_interval, filter_fwhm_interval, fabry_perot_fwhm_interval, \
    hrs_interval, imaging_energy_properties, rss_energy_properties, rss_slit_width_from_barcode, rss_resolution_element
...
def test_fabry_perot_fwhm_calculation(mocker):
    with mocker.patch.object(
        ...
        assert fabry_perot_fwhm_interval(resolution="low resolution", wavelength=3.5) == 3

    with mocker.patch.object(
            ReadInstrumentWavelength, 'fp_hwfm',
            return_value=[("HR", 0, 0), ("HR", 1, 1), ("HR", 2, 3), ("HR", 3, 4), ("HR", 4, 3), ("HR", 5, 4),
                          ("HR", 6, 2)]) as mm:
        assert fabry_perot_fwhm_interval(resolution="high resolution", wavelength=3) == 4
        assert fabry_perot_fwhm_interval(resolution="high resolution", wavelength=4) == 3
        assert fabry_perot_fwhm_interval(resolution="high resolution", wavelength=5.5) == 3
...
def test_imaging_mode_cal(mocked_function):
    ...
    tes = imaging_energy_properties(plane_id=1, filter_name="method using filter is mocked", instrument=types.Instrument.RSS)
...
def test_get_resolution_element():
    assert rss_resolution_element(grating_angle=30 * u.deg, grating_frequency=1 * 1 / u.mm, slit_width=5 * u.arcsec) \
           == 15394.902011247219
    assert rss_resolution_element(grating_angle=30 * u.deg, grating_frequency=1 * 1 / u.mm, slit_width=.5 * u.arcsec) \
           == 1539.4902011247218
    assert rss_resolution_element(grating_angle=((30 * math.pi) / 180) * u.rad, grating_frequency=1 * 1 / u.mm,
